---
name: skillopt-sleep
description: "Use when the user wants Codex to self-improve from past usage, asks about a nightly/offline 'sleep' or 'dream' cycle, wants Codex to review archived sessions, learn preferences, consolidate memory/skills, run dry-run/run/adopt/status for SkillOpt-Sleep, or schedule offline self-optimization. Drives the skillopt_sleep engine: harvest Codex Desktop sessions -> mine recurring tasks -> replay offline -> consolidate validated memory + skills behind a held-out gate."
---
...
offline or on demand, it reviews real Codex Desktop archived sessions, re-runs
recurring tasks on the user's own budget, and consolidates what it learns into
memory and skills. It keeps only changes that pass a held-out validation gate,
and live files change only after the user explicitly adopts a staged proposal.
There is no model-weight training.
...
- to review Codex Desktop archived sessions and distill recurring tasks;
- to consolidate feedback into memory or managed skills;
- to run `status`, `harvest`, `dry-run`, `run`, or `adopt` for SkillOpt-Sleep.

## The cycle

1. **Harvest** — read Codex Desktop archives from
   `~/.codex/archived_sessions` read-only and normalize them into session
   digests.
...
bash "$SKILLOPT_SLEEP_REPO/plugins/run-sleep.sh" status --project "$(pwd)" --source codex
bash "$SKILLOPT_SLEEP_REPO/plugins/run-sleep.sh" harvest --project "$(pwd)" --source codex
bash "$SKILLOPT_SLEEP_REPO/plugins/run-sleep.sh" dry-run --project "$(pwd)" --source codex --backend mock
bash "$SKILLOPT_SLEEP_REPO/plugins/run-sleep.sh" run --project "$(pwd)" --source codex --backend codex --max-sessions 5 --max-tasks 3 --progress
bash "$SKILLOPT_SLEEP_REPO/plugins/run-sleep.sh" run --project "$(pwd)" --source codex --backend codex --target-skill-path .agents/skills/example/SKILL.md --max-sessions 5 --max-tasks 3 --progress
bash "$SKILLOPT_SLEEP_REPO/plugins/run-sleep.sh" adopt --project "$(pwd)" --source codex
```
...
- `--codex-home /path/to/.codex` points at a non-default archive location.
...
- Bound real Codex runs with `--max-sessions` and `--max-tasks`; use
  `--progress` because child `codex exec` calls may be quiet for minutes.
- Use `--target-skill-path .agents/skills/<name>/SKILL.md` to stage/adopt a
  repo-scoped Codex skill instead of the default managed user-level skill.
...
```bash
python -m skillopt_sleep dry-run --project "$(pwd)" --source codex --backend mock --json
